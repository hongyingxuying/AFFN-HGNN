...
from utils.train_graph_ronghe import train_utils
...
def parse_args():
    ...
    parser.add_argument('--data_name', type=str, default='PUPathronghe', help='the name of the data')
    parser.add_argument('--input_type', choices=['TD', 'FD', 'other'], type=str, default='FD',
                        help='the input type decides the length of input')
    parser.add_argument('--data_dir', type=str, default="../GNNforIFD/data/PU",
                        help='the directory of the data')
    parser.add_argument('--cuda_device', type=str, default='0', help='assign device')
    parser.add_argument('--checkpoint_dir', type=str, default='E:/实验模型数据/GNNforIFD/checkpoint', help='the directory to save the model')
...
if __name__ == '__main__':
    ...
    if args.task == 'Node':
        sub_dir = args.task + '_' + args.model_name + '_' + args.data_name + '_' + args.input_type + '_' + datetime.strftime(
            datetime.now(), '%m%d-%H%M%S')
    else:
        sub_dir = args.task + '_' + args.model_name + '_' + args.pooltype + '_' + args.data_name + '_' + args.input_type + '_' + datetime.strftime(
            datetime.now(), '%m%d-%H%M%S')
