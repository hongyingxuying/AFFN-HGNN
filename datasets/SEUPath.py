...
from datasets.AuxFunction import FFT, wavelet_packet_transform
import pickle
...
# Gdata = ["Chipped_30_2.csv", "Health_30_2.csv", "Miss_30_2.csv", "Root_30_2.csv", "Surface_30_2.csv"]
Gdata = ["Chipped_30_2.csv", "Health_30_2.csv", "Miss_30_2.csv", "Root_30_2.csv"]
...
def get_files(sample_length, root, InputType, task, test=False):
    ...
    '''
    datasetname = os.listdir(os.path.join(root, os.listdir(root)[1]))  # 0:bearingset, 2:gearset
    root1 = os.path.join(root, os.listdir(root)[1], datasetname[0])  # Path of bearingset
...
def data_load(signal_size, filename, dataname, label, InputType, task):
    ...
    graphset1 = pathGraph(10, data, label, task)
    graphset2 = KNNGraph(10, data, label, task)
    graphset3 = RadiusGraph(10, data, label, task)
